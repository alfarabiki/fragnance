...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.isMandatory !== undefined) fields.is_mandatory = body.isMandatory;
  if (body.isActive !== undefined) fields.is_active = body.isActive;

  if (Object.keys(fields).length > 0) {
    const { error } = await client.from("packaging").update(fields).eq("id", id);
    if (error) return NextResponse.json({ error: { message: error.message } }, { status: 500 });
  }

  if (body.currentStock !== undefined) {
    const { error } = await upsertStock(client, "PACKAGING", id, body.currentStock);
